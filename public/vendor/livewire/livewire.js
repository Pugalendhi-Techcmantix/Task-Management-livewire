            ...
            (function (root, factory) {
                ...
                var css = (function () {
                    ...
                    function getStyleProp(name) {
                        ...
                        return (
                            cssProps[name] ||
                            (cssProps[name] = getVendorProp(name))
                        );
                    }
...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`,
    ...
    wireProperty(
        "$set",
        (component) =>
            async (property, value, live = true) => {
    ...
    var Component = class {
        constructor(el) {
            ...
            if (!this.snapshot) {
                throw (
                    `Snapshot missing on Livewire component with id: ` + this.id
                );
            }
    ...
    var DirectiveManager = class {
        ...
        get(value) {
            return this.directives.find(
                (directive3) => directive3.value === value,
            );
        }
        ...
        var getInitialFocusNode = function getInitialFocusNode2() {
            ...
            if (node === void 0) {
                if (findContainerIndex(doc.activeElement) >= 0) {
                    ...
                    node =
                        firstTabbableNode || getNodeForOption("fallbackFocus");
                }
            }
            if (!node) {
                throw new Error(
        ...
        Alpine3.directive(
            ...
            Alpine3.skipDuringClone(
                ...
                (
                    ...
                    if (el._x_anchor) {
                        unstyled ||
                            setStyles2(el, el._x_anchor.x, el._x_anchor.y);
                    }
                },
            ),
        );
    ...
    function updateUrlAndStoreLatestHtmlForFutureBackButtons(
        html,
        destination,
    ) {
    ...
    function ifTheQueryStringChangedSinceLastRequest(el, currentHeadChildren) {
        ...
        return currentHeadChildren.some((child) => {
            ...
            let [currentUri, currentQueryString] =
                extractUriAndQueryString(child);
    ...
    on2("payload.intercept", async ({ assets }) => {
        ...
        for (let [key, asset] of Object.entries(assets)) {
            await onlyIfAssetsHaventBeenLoadedAlreadyOnThisPage(
                key,
                async () => {
    ...
    async function onlyIfAssetsHaventBeenLoadedAlreadyOnThisPage(
        key,
        callback,
    ) {
        if (executedAssets.has(key)) return;
        ...
        ({ el, directive: directive3, cleanup: cleanup2, component }) =>
            setTimeout(() => {
                ...
                el.addEventListener("submit", () => {
                    let componentId = directive3.expression.startsWith(
                        "$parent",
                    )
                        ? component.parent.id
                        : component.id;
    ...
    on2("effect", ({ component, effects, cleanup: cleanup2 }) => {
        ...
        Object.entries(queryString).forEach(([key, value]) => {
            let { name, as, use, alwaysShow, except } =
                normalizeQueryStringEntry(key, value);
            if (!as) as = name;
        ...
        listeners2.forEach((event) => {
            if (event.startsWith("echo")) {
                ...
                if (
                    ...
                    window.Echo[channel_type](channel).listen(
                        event_name,
                        handler4,
                    );
                    component.addCleanup(() => {
    ...
    directive2(
        ...
        ({ el, directive: directive3, component, cleanup: cleanup2 }) => {
            let visibility = module_default.reactive({
            ...
            let cleanups2 = [];
            cleanups2.push(
                on2("morph.removing", ({ el: el2, skip }) => {
                    skip();
        ...
        if (directive3.modifiers.includes("class")) {
            ...
            let display =
                [
                    ...
                    "grid",
                    "inline-flex",
                ].filter((i) => directive3.modifiers.includes(i))[0] ||
                "inline-block";
            display =
                directive3.modifiers.includes("remove") && !isTruthy
                    ? cache
                    : display;
    ...
    function getTargets(el) {
        ...
        if (directives2.has("target")) {
            ...
            directives2
                ...
                .filter(
                    (i) =>
                        !nonActionOrModelLivewireDirectives.includes(i.value),
                )
    ...
    on2("request", ({ respond }) => {
        respond((mutableObject) => {
            ...
            mutableObject.response = {
                ...
                async text() {
                    let finalResponse =
                        await interceptStreamAndReturnFinalResponse(
                            response,
                            (streamed) => {
    ...
    directive2("poll", ({ el, directive: directive3 }) => {
        ...
        throttleWhile(
            () =>
                ...
                theDirectiveIsMissingKeepAlive(directive3),
        );
        pauseWhile(
            () =>
                theDirectiveHasVisible(directive3) &&
